...
def create_file_with_duplicates(file_name, duplicates, field_index)
  count_empty_values = 0

  CSV.open(file_name, 'w') do |csv_duplicate_object|
    duplicates.each do |row_duplicate_array|
      csv_duplicate_object << row_duplicate_array
      count_empty_values += 1 if row_duplicate_array[field_index].nil?
    end
  end
...
count_empty_ean_gtin = create_file_with_duplicates(
  'ready_csv/with_ean_gtin_duplicates.csv',
  array_ean_gtin_duplicates,
  ean_or_gtin
)
...
count_empty_ean_gtin = create_file_with_duplicates(
  'ready_csv/with_id_duplicates.csv',
  array_id_duplicates,
  id
)
